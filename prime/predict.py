...
def main():
    ...
    parser.add_argument('--pdb', nargs='+', required=True, type=str, help='List of PDB files, as file.cif[:chain]')
    ...
    parser.add_argument("--ckpt", type=str, help="Checkpoint file to structure model, downloaded for --metal if omitted", default=None)
    ...
    parser.add_argument("--stats", type=str, default=paths.stats_csv(), help="Statistics file of metal-NOS-AA distances")
    ...
    if args.ckpt is None:
        if args.metal is None:
            parser.error("either --metal or --ckpt is required")
        args.ckpt = paths.probe_ckpt(args.metal)
    if args.metal is None:
    ...
    probe_args = Config(paths.config(f"seq_predictors/{args.metal}.yaml"))
    train_structure_args = Config(paths.config('train_structure_predictor.yaml'))
    ...
    state_dict = torch.load(
        paths.seq_ckpt(args.metal), dev, weights_only=True)["state_dict"]
