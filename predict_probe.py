...
def main():
    ...
    parser.add_argument('--pdb', nargs='+', type=str, help='List of PDB files', default=['12ca.cif'])
    ...
    parser.add_argument("--ckpt", type=str, help="Checkpoint file to structure model", default='checkpoints/tune_probe_predictor/probe_ZN_resnet152_pretrained=True_hard_mining_epoch=26_val_auc=0.9910.ckpt')
    ...
    for pdb_idx, pdb_file in enumerate(args.pdb):
        ...
        for chain_id, (seq, pred, prob) in candidates.items():
            ...
            probe_data['seq2res'] = deepcopy((seq2res, pred, prob))
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16), torch.inference_mode():
            ...
            probe_data['probes'] = deepcopy(probes.cpu().numpy())
            probe_data['kskps'] = deepcopy(kskps.cpu().numpy())
        ...
        if len(y_pred_offsets) == 1:
            ...
            groups = fclusterdata(
                y_pred_offsets.cpu().numpy(), clust_dist, criterion='distance', method='complete')
        ...
        for center_idx in centers:
            ...
            all_grouping.append((cluster, prob, bf))
            if len(cluster) <= 1:
                continue
        ...
        probe_data['all_grouping'] = all_grouping
        probe_data['predictions'] = predictions
        ...
        torch.save(
            probe_data, f'notebooks/{Path(pdb_file).stem}-probe_data.pt')
        atoms = []
